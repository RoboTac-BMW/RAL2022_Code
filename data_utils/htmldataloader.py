...
def load_data(dir,classification = False, train_pts='random', test_pts=1024):
    ...
    if classification:
        ...
        for i in range(num_train):
            if train_pts == 'random':
                sample_size = random.choice([10,20,30,40,50])
            else:
                sample_size = train_pts
            num_of_rows = train_data[i].shape[0]
            random_indices = np.random.choice(num_of_rows, size=sample_size, replace=False)
            sampled_train_data[i] = train_data[i][random_indices, :]
        print(sampled_train_data.shape)

        for i in range(num_test):
            num_of_rows = test_data[i].shape[0]
            random_indices = np.random.choice(num_of_rows, size=test_pts, replace=False)
...
class ModelNetDataLoader(Dataset):
    def __init__(self, data, labels, point_num=1024, rot=True, use_buffer=True, use_voxel=False, rot_type="z", point_sample=False):
