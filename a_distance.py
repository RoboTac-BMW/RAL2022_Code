...
def calculate(source_feature: torch.Tensor, target_feature: torch.Tensor,
              device, progress=True, training_epochs=500):
    """
    ...
    optimizer = SGD(anet.parameters(), lr=0.0001)
    a_distance = 2.0
...
if __name__ == '__main__':
    ...
    with open ('dist_calc/vision_train_afterDA.npy', 'rb') as f:
     vision_mat = np.load(f)
    with open ('dist_calc/tactile_train_afterDA.npy', 'rb') as f_:
     tactile_mat = np.load(f_)

    print(vision_mat.shape)
    print(tactile_mat.shape)

    vision_mat_tensor = torch.from_numpy(vision_mat)
    tactile_mat_tensor = torch.from_numpy(tactile_mat)
    a_dist = calculate(vision_mat_tensor.float(), tactile_mat_tensor.float(), device='cuda:0')
    # print(a_dist)
