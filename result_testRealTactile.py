...
def test(model, loader, num_class=15, vote_num=1):
    ...
    for j, data in tqdm(enumerate(loader), total=len(loader)):
        ...
        # print("................................")
        tmp_label = preds_new.data.cpu().numpy()[0]
        # print(tmp_label)
        ...
        if tmp_label == 0 or tmp_label == 1 or tmp_label == 8:
            _, top2label = torch.topk(output_new.data, 2, dim=1)
            new_label = top2label.data.cpu().numpy()[0][1]
            # print("!!!!!!!!!!!!!!!!!!!!!")
            # print(tmp_label)
            # print(new_label)
            if new_label == 0 or new_label == 1 or new_label == 8:
                _, top3label = torch.topk(output_new.data, 3, dim=1)
                new_label = top3label.data.cpu().numpy()[0][2]

            y_pred_new = np.array([new_label])
            # if y_pred_new [0] == 0:
            #     print("!!!!!!!!")
            #     print(top2label.data.cpu().numpy())
    ...
    cf_matrix_new = np.delete(cf_matrix_new, 0, 0)
    cf_matrix_new = np.delete(cf_matrix_new, 0, 0)
    cf_matrix_new = np.delete(cf_matrix_new, 0, 1)
    cf_matrix_new = np.delete(cf_matrix_new, 0, 1)
    cf_matrix_new = np.delete(cf_matrix_new, 6, 0)
    cf_matrix_new = np.delete(cf_matrix_new, 6, 1)
...
def main(args):
    ...
    tactile_data_path = 'data/tactile_pcd_10_sampled_15-classes/'
    ...
    print(classes)
    print(classes.keys)
    tmp_classes = ['cleaner', 'coffee', 'cup', 'eraser', 'glasses_box', 'jam', 'olive_oil', 'shampoo', 'spray', 'sugar', 'tape', 'wine']
    ...
    with torch.no_grad():
        ...
        # Draw confusion matrix
        df_cm = pd.DataFrame(cf_matrix_new, index = [i for i in tmp_classes], columns = [i for i in tmp_classes])
