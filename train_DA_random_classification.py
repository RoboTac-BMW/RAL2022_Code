...
from htmldataloader import load_data, ModelNetDataLoader
...
def parse_args():
    ...
    parser.add_argument('--num_sparse_point', type=int, default=0, help='Point Number for domain loss')
...
def main(args):
    ...
    data_path = r'./data/modelnet40_ply_hdf5_2048/'

    ##############################PLY Random##################################
    train_data, train_label, test_data, test_label = load_data(data_path, classification=True
                                                               train_pts=1024, test_pts=1024)
    train_DA_data, train_DA_label, _, _ = load_data(data_path, classification=True
                                                    train_pts='random')

    trainDataset = ModelNetDataLoader(train_data, train_label, use_voxel=False, rot_type='z')
    trainDA_Dataset = ModelNetDataLoader(train_DA_data, train_DA_label, use_voxel=False, rot_type='z')
    testDataset = ModelNetDataLoader(test_data, test_label, use_voxel=False, rot_type='z')

    trainDataLoader = torch.utils.data.DataLoader(trainDataset, batch_size=args.batch_size, shuffle=True)
    trainDA_DataLoader = torch.utils.data.DataLoader(trainDA_Dataset, batch_size=args.batch_size, shuffle=True)
    testDataLoader = torch.utils.data.DataLoader(testDataset, batch_size=args.batch_size, shuffle=True) #, num_workers = 6
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        #         smoothing=0.9):
        for batch_id, (data, data_DA) in tqdm(
                enumerate(zip(trainDataLoader, trainDA_DataLoader), 0),
                toral=len(trainDataLoader)
                smoothing=0.9):

            optimizer.zero_grad()

            points, norms, target = data
            target = target [:, 0]
            target = target.to(device)

            points = np.concatenate((points,norm),axis=1)
            points = points.to(device)

            points_DA, norms_DA, _ = data_DA
            points_DA = np.concatenate((points_DA ,norm_DA),axis=1)
            points_DA = points_DA.to(device)
